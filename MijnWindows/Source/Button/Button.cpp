...
Button::Button(Window * aParent, HINSTANCE hInstance, const std::tstring & aName,
               BUTTON_STRATEGY aStrategy)
: MijnWindow<Button>(aParent, hInstance, aName),
  mStrategy         (NULL),
  mPressed          (false)
{
  Create(GetClassName(), _T("Button"), WS_CHILD);
...
ButtonStrategy * Button::MakeStrategy(BUTTON_STRATEGY aStrategy)
{
  switch (aStrategy) {
    ...
    case BS_ToggleButton:
      return new ToggleButtonStrategy(*this);
    case BS_CustomButton:
