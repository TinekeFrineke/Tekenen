...
Window::Window(Window* aParent, HINSTANCE hInstance, const std::string& aName)
    : mParent(aParent),
...
#if PRINT_MESSAGES > 1
    TCHAR msg[1024];
    _stprintf(msg, "Constructing window %s address = 0x%08x parent == %s\n"),
              aName.c_str(), this, aParent ? aParent->mName.c_str() : "NULL"));
...
#if PRINT_MESSAGES > 1
    TCHAR msg[1024];
    _stprintf(msg, "Destructor window %s 0x%08x 1\n"), mName.c_str(), this);
    OutputDebugString(msg);
#endif
...
#if PRINT_MESSAGES > 1
    _stprintf(msg, "Destructor window %s 2\n"), mName.c_str());
    OutputDebugString(msg);
#endif
}
...
bool Window::Create(const std::string& aClassName,
...
{
    FILE* fp(NULL);
    errno_t err(fopen_s(&fp, "C:\\Tineke\\tineke.txt", "a")); (void)err;
    if (!fp)
        ::MessageBox(0, "fp == NULL", "ERROR", MB_OK);

    if (fp) {
        fprintf(fp, "Window::Create(\"%s\", \"%s\", 0x%08x, 0x%08x, %d, %d, %d, %d, NULL 0x%08p)\n",
                  aClassName.c_str(), aName.c_str(), aStyle, anExStyle, aLeft, aTop, aWidth, aHeight, aParam);
    }
    if (mHwnd != NULL)
    {
        ::MessageBox(0, ("Double create for window " + mName).c_str(), "ERROR", MB_OK);
        throw 0;
    }

    if (fp) {
        fprintf(fp, "CreateWindowEx(0x%08x %s %s 0x%08x %d %d %d %d 0x%08p)\n",
...
#if PRINT_MESSAGES > 1
    TCHAR msg[1024];
    _stprintf(msg, "Creating window %s HWND == 0x%08x\n"), mName.c_str(), mHwnd);
    OutputDebugString(msg);
#endif

    if (mHwnd == NULL)
    {
        ::MessageBox(0, ("Error creating window " + mName).c_str(), "ERROR", MB_OK);
    }
...
#if PRINT_MESSAGES > 1
    TCHAR smsg[1024];
    _stprintf(smsg, "Created window HWND == 0x%08x class == %s name == %s\n"),
...
#pragma warning (pop)
    if (wnd != NULL)
    {
        ...
        if (wnd->mDebug)
        {
            ...
            {
                case WM_CHAR:
                    _stprintf(msgname, "WM_CHAR"));
                    break;
                case WM_PAINT:
                    _stprintf(msgname, "WM_PAINT"));
                    break;
                case WM_LBUTTONDOWN:
                    _stprintf(msgname, "WM_LBUTTONDOWN"));
                    break;
                case WM_LBUTTONUP:
                    _stprintf(msgname, "WM_LBUTTONUP"));
                    break;
                case WM_PARENTNOTIFY:
                    _stprintf(msgname, "WM_PARENTNOTIFY"));
                    break;
                case WM_SETCURSOR:
                    _stprintf(msgname, "WM_SETCURSOR"));
                    break;
                case WM_NCHITTEST:
                    _stprintf(msgname, "WM_NCHITTEST"));
                    break;
                case WM_MOUSEACTIVATE:
                    _stprintf(msgname, "WM_MOUSEACTIVATE"));
                    break;
                case WM_MOUSEMOVE:
                    _stprintf(msgname, "WM_MOUSEMOVE"));
                    break;
                case WM_KEYUP:
                    _stprintf(msgname, "WM_KEYUP"));
                    break;
                case WM_SYSKEYDOWN:
                    _stprintf(msgname, "WM_SYSKEYDOWN"));
                    break;
                case WM_CANCELMODE:
                    _stprintf(msgname, "WM_CANCELMODE"));
                    break;
                case WM_SYSKEYUP:
                    _stprintf(msgname, "WM_SYSKEYUP"));
                    break;
                case WM_NCACTIVATE:
                    _stprintf(msgname, "WM_NCACTIVATE"));
                    break;
                case WM_GETTEXT:
                    _stprintf(msgname, "WM_GETTEXT"));
                    break;
                case WM_ACTIVATE:
                    _stprintf(msgname, "WM_ACTIVATE"));
                    break;
                case WM_ACTIVATEAPP:
                    _stprintf(msgname, "WM_ACTIVATEAPP"));
                    break;
                case WM_WINDOWPOSCHANGING:
                    _stprintf(msgname, "WM_WINDOWPOSCHANGING"));
                    break;
                case WM_WINDOWPOSCHANGED:
                    _stprintf(msgname, "WM_WINDOWPOSCHANGED"));
                    break;
                case WM_NCCALCSIZE:
                    _stprintf(msgname, "WM_NCCALCSIZE"));
                    break;
                case WM_CHILDACTIVATE:
                    _stprintf(msgname, "WM_CHILDACTIVATE"));
                    break;
                case WM_KILLFOCUS:
                    _stprintf(msgname, "WM_KILLFOCUS"));
                    break;
                case WM_SETFOCUS:
                    _stprintf(msgname, "WM_SETFOCUS"));
                    break;
                case WM_ERASEBKGND:
                    _stprintf(msgname, "WM_ERASEBKGND"));
                    break;
                case WM_NCPAINT:
                    _stprintf(msgname, "WM_NCPAINT"));
                    break;
                case WM_CAPTURECHANGED:
                    _stprintf(msgname, "WM_CAPTURECHANGED"));
                    break;
                default:
                    _stprintf(msgname, "0x%08x"), uMsg);
            }

            TCHAR msg[1024];
            _stprintf(msg, "WindowProc for message %s window %s address = 0x%08x\n"),
...
GenericWindow::GenericWindow(Window* aParent, HINSTANCE hInstance,
                             const std::string& aName, DWORD dwStyle, DWORD dwExStyle)
