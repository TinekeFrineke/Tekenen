...
bool HorizontalFixedSizeStrategy::Layout(const RECT& aRect)
{
    ...
    for (size_t i = 0; i < GetButtons().size(); ++i)
        GetButtons()[i]->SetWindowPos(nullptr, 2 + int(i) * (GetButtonHeight() + 2), 2,
...
bool VerticalFixedSizeStrategy::Layout(const RECT& aRect)
{
    (void)aRect;
    for (size_t i = 0; i < GetButtons().size(); ++i)
        GetButtons()[i]->SetWindowPos(nullptr, 2, 2 + int(i) * (GetButtonWidth() + 2),
                                      GetButtonWidth(), GetButtonHeight(), SWP_SHOWWINDOW);
    return true;
}
...
bool HorizontalFitStrategy::Layout(const RECT& aRect)
{
    ...
    for (size_t i = 0; i < GetButtons().size(); ++i)
        GetButtons()[i]->SetWindowPos(nullptr, 2 + int(i) * (iWidth + 2), 2,
...
bool VerticalFitStrategy::Layout(const RECT& aRect)
{
    ...
    for (size_t i = 0; i < GetButtons().size(); ++i)
        GetButtons()[i]->SetWindowPos(nullptr, 2, 2 + int(i) * (iHeight + 2),
                                      iWidth, iHeight, SWP_SHOWWINDOW);
    return true;
}
...
bool ButtonPanel::RegisterClass(HINSTANCE hInstance)
{
    ...
    //wndclass.hCursor = LoadCursor(nullptr, MAKEINTRESOURCE(IDC_ARROW));

    return ::RegisterClass(&wndclass) != NULL;
}


void ButtonPanel::AddButton(Button* aButton, int /*id*/)
{
...
{
    for (size_t i = 0; i < GetButtons().size(); ++i)
        GetButtons()[i]->SetWindowPos(nullptr, 2 + int(i) * 50, 2,
                                      48, 48, oPos.flags);
...
ButtonBar::ButtonBar(Window* aParent, HINSTANCE hInstance,
    ...
    mStrategy(nullptr)
{
    Create(GetClassName(), "MijnButtonBar", WS_CHILD);

    switch (aStrategy)
    {
        case LAYOUT_STRATEGY::HorizontalFixedSize:
            mStrategy = new HorizontalFixedSizeStrategy(*this, 48, 48);
            break;

        case LAYOUT_STRATEGY::VerticalFixedSize:
            mStrategy = new VerticalFixedSizeStrategy(*this, 48, 48);
            break;

        case LAYOUT_STRATEGY::HorizontalFit:
            mStrategy = new HorizontalFitStrategy(*this);
            break;

        case LAYOUT_STRATEGY::VerticalFit:
