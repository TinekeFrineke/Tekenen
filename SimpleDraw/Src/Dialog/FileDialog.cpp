...
FileDialog::FileDialog(Window* aParent, HINSTANCE hInstance,
                       bool bExistingOnly)
    : Dialog(aParent, hInstance/*, "FileDialig"), WS_THICKFRAME*/),
    mOKButton(NULL),
    mButtonUp(NULL),
    mButtonDown(NULL),
    mButtonBar(NULL),
    ...
    mFirstIndex(0)
{
    ...
    mCloseButton = new DoubleBitmapButton(this, hInstance, true, "ButtonClose");
    ...
    mFileControl = new FileControl(this, hInstance, bExistingOnly, "FileControl");
...
void FileDialog::SelectUser(const std::string& aUser)
{
    mFileMask = Profile::GetInstance().GetDataDirectory() + "\\" + aUser + "*.bmp";
    mEntries.clear();
    ...
    if (hFile != NULL)
        if (data.dwFileAttributes != FILE_ATTRIBUTE_DIRECTORY)
            mEntries.push_back(Profile::GetInstance().GetDataDirectory() + "\\" + data.cFileName);
