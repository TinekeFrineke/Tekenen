...
: MijnWindow<MainWindow>(NULL, hInstance, _T("The MainWindow")),
  ...
  mRedoButton           (NULL)
{
  Create(GetClassName(), _T("Ik Teken"),
#ifdef NDEBUG
    WS_POPUP
#else
    WS_BORDER
  ...
  mFileButtonBar  = new ButtonBar(this, hInstance, _T("FileButtonBar"),
                                  ButtonBar::LS_HorizontalFixedSize);

  mEraseButton    = new DoubleBitmapButton(mFileButtonBar, hInstance, true, _T("EraseButton"));
  mSaveButton     = new DoubleBitmapButton(mFileButtonBar, hInstance, true, _T("SaveButton"));
  mLoadButton     = new DoubleBitmapButton(mFileButtonBar, hInstance, true, _T("LoadButton"));

  mUndoButtonBar  = new ButtonBar(this, hInstance, _T("UndoButtonBar"),
                                  ButtonBar::LS_HorizontalFixedSize);
  mUndoButton     = new DoubleBitmapButton(mUndoButtonBar, hInstance, true, _T("UndoButton"));
  if (REDO)
    mRedoButton     = new DoubleBitmapButton(mUndoButtonBar, hInstance, true, _T("RedoButton"));

  mCloseButton    = new DoubleBitmapButton(this, hInstance, true, _T("CloseButton"));

  mEraseButton->SetBitmap (IDB_ERASE, IDB_ERASE_PRESSED);
  mSaveButton->SetBitmap  (IDB_SAVE,  IDB_SAVE_PRESSED);
  mLoadButton->SetBitmap  (IDB_LOAD,  IDB_LOAD_PRESSED);
  ...
  mUndoButton->SetBitmap  (IDB_UNDO,  IDB_UNDO_PRESSED);
  if (REDO)
    mRedoButton->SetBitmap  (IDB_REDO,  IDB_REDO_PRESSED);
  ...
  {
    ::MessageBox(0, L"Error creating main window", L"ERROR", MB_OK);
    throw 0;
  }
