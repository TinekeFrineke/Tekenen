...
HoverWindow::HoverWindow(Window* aParent, HINSTANCE hInstance, StampState& aState,
    ...
    : GenericWindow(aParent, hInstance, "HoverWindow", WS_POPUP, WS_EX_TRANSPARENT),
    mState(aState),
...
void HoverWindow::SetState(STATE aState)
{
    ...
    switch (aState)
    {
        case ST_Position:
            mSubState = new StampPositionState(mState, *this);
            OutputDebugString("Set ST_Position\n");
            break;

        case ST_Size:
            mSubState = new StampSizeState(mState, *this);
            OutputDebugString("Set ST_Size\n");
            break;

        case ST_Ready:
            mSubState = nullptr;
            mState.SetHoverWindow(nullptr);
            OutputDebugString("Set ST_Ready\n");
            break;
        default:
            assert(false);
            mSubState = new StampPositionState(mState, *this);
